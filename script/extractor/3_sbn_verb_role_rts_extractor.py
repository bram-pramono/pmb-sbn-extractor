...
def extract_verbs_and_role_distances(non_connector_sbn_lines):
  ...
  for idx, verb_line in sbn_verb_lines:
    ...
    negative_distance_roles = [(x, int(y)) for x, y in verb_line.roles if y.startswith('-')]
...
if __name__ == '__main__':
  ...
  for filename in sorted(os.listdir(sbn_drs_folder)):
    ...
    with open(src) as scraped_file:
      ...
      if not sbn_parsing_in_order:
        ...
        sbn_verb_role_data = extract_verbs_and_role_distances(non_connector_sbn_lines)
        sent_nr = int(sent_nr)
        for verb_role_data in sbn_verb_role_data:
          word_pos = verb_role_data['word_pos']
          spr_rt_all_subj = spr_data_with_subj[sent_nr][word_pos]
          for subj_nr, rt in spr_rt_all_subj.items():
            spr_copy_role_data = copy.deepcopy(verb_role_data)
            spr_copy_role_data['subj_nr'] = subj_nr
            spr_spill = -999
            if word_pos + 1 in spr_data_with_subj[sent_nr]:
              spr_spill = spr_data_with_subj[sent_nr][word_pos + 1][subj_nr]

            spr_copy_role_data['rt'] = rt
            spr_copy_role_data['spill'] = spr_spill
            extracted_spr_sbn_data_with_subj.append(spr_copy_role_data)

          et_rt_all_subj = et_data_with_subj[sent_nr][word_pos]
          for subj_nr, rt in et_rt_all_subj.items():
            et_copy_role_data = copy.deepcopy(verb_role_data)
            et_copy_role_data['subj_nr'] = subj_nr
            et_spill_ff = -999
            et_spill_fp = -999
            et_spill_rb = -999
            et_spill_gp = -999
            ...
            et_copy_role_data['rt_ff'] = et_data_with_subj[sent_nr][word_pos][subj_nr][0]
            et_copy_role_data['rt_fp'] = et_data_with_subj[sent_nr][word_pos][subj_nr][1]
            et_copy_role_data['rt_rb'] = et_data_with_subj[sent_nr][word_pos][subj_nr][2]
            et_copy_role_data['rt_gp'] = et_data_with_subj[sent_nr][word_pos][subj_nr][3]
            et_copy_role_data['spill_ff'] = et_spill_ff
            et_copy_role_data['spill_fp'] = et_spill_fp
            et_copy_role_data['spill_rb'] = et_spill_rb
            et_copy_role_data['spill_gp'] = et_spill_gp
            extracted_et_sbn_data_with_subj.append(et_copy_role_data)

  wrap_print(f'{len(skipped_pmb_ids)} pmb records are skipped. The whole list can be seen below:')
  wrap_print(str(skipped_pmb_ids))

  with open(to_abspath(data_folder, f'sbn_verb_role_spr_per_subj_rts.tsv'), 'w') as out_file:
    w = csv.DictWriter(out_file, extracted_spr_sbn_data_with_subj[0].keys(), delimiter='\t')
    w.writeheader()
    w.writerows(extracted_spr_sbn_data_with_subj)

  with open(to_abspath(data_folder, f'sbn_verb_role_et_per_subj_rts.tsv'), 'w') as out_file:
  ...
  flush_clean_wrap_print(to_abspath(report_folder, f'3_sbn_verb_role_rts_extractor_{today}.txt'))
